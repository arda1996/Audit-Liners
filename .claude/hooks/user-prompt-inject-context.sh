...
# Dedup guard: if this hook fires more than once for the same prompt, inject only
# once (prevents double-injection / wasted tokens). Marker keyed by prompt hash.
mkdir -p ".claude/sessions"
PROMPT_HASH=$(printf '%s' "$PROMPT" | shasum 2>/dev/null | cut -c1-16)
DEDUP_MARKER=".claude/sessions/.injected-${PROMPT_HASH}"
if [ -n "$PROMPT_HASH" ] && [ -f "$DEDUP_MARKER" ]; then
  exit 0
fi
[ -n "$PROMPT_HASH" ] && touch "$DEDUP_MARKER"
# Keep only the 20 most recent dedup markers
ls -1t .claude/sessions/.injected-* 2>/dev/null | tail -n +21 | xargs rm -f 2>/dev/null
