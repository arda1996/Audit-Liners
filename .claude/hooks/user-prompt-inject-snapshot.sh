...
# Cumulative brain pointer — cheap, always injected if present
if [ -f "$CONTEXT" ]; then
  echo "--- Süreklilik Çıpası ---"
  echo "📎 Kümülatif proje beyni: .claude/CONTEXT.md — bağlam koparsa/özetlenince ÖNCE bunu oku."
  if [ -f "$JOURNAL" ]; then
    echo ""
    echo "Son karar günlüğü:"
    grep '^- ' "$JOURNAL" 2>/dev/null | tail -5
  fi
  echo "--- ---"
  echo ""
fi
