...
# Extract file_path from captured stdin JSON
...
if [ "$FILE_BYTES" -ge "$READ_MAX_BYTES" ] 2>/dev/null; then
  # Advisory only (exit 0): blocking legitimate large-source reads harms real work.
  FILE_KB=$((FILE_BYTES / 1024))
  APPROX_TOKENS=$((FILE_BYTES / 4))
  echo "⚠️  Large read: '$FILE_PATH' is ${FILE_KB}KB (~${APPROX_TOKENS} tokens)." >&2
  echo "   Prefer Read with offset/limit, or grep to locate the relevant section first." >&2
  exit 0
